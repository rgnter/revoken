...
public class ReportController extends AController {
    ...
    public void initialize() throws Exception {
        ...
        Bukkit.getCommandMap().register("revoken", new Command("playerreport", "Used to report a player", "/playerreport <player name> <description>", Arrays.asList("reportplayer", "reportuser", "hacker")) {
            @Override
            public boolean execute(@NotNull CommandSender commandSender, @NotNull String s, @NotNull String[] args) {
                if(!commandSender.hasPermission("revoken.report"))
                    return true;
    ...
    private @NotNull DiscordWebhook.EmbedObject commonEmbed(@NotNull Player player, @NotNull String message, long id, long reportUnixTimestamp) {
        return new DiscordWebhook.EmbedObject().setColor(new Color(230, 95, 95))
                .setAuthor(player.getName(),  "", ApiConnector.getSkinHeadUrl(player))
